...
const DISCORD_LOG_LINE_RE = /discord: inbound id=(?<messageId>\d+)\s+guild=(?<guildId>\S+)\s+channel=(?<channelId>\d+)\s+mention=(?<mention>yes|no)\s+type=(?<type>\S+)\s+content=(?<content>yes|no)/u;
const userCardsConfigSchema = {
    ...
    properties: {
        ...
        passiveDiscordLogTail: {
            type: "object",
            description: "Local debug-log tailer that hydrates Discord message authors before mention-gated drops.",
            properties: {
                enabled: {
                    type: "boolean",
                    default: false,
        ...
        passiveDiscordGateway: {
            type: "object",
            description: "Local Discord gateway tap that captures author id/display name without turning passive chatter into agent turns.",
            properties: {
                enabled: {
                    type: "boolean",
                    default: false,
                ...
                captureMessages: {
                    type: "boolean",
                    description: "Store sanitized recent message snippets from the passive tap. Requires Discord Message Content intent for most guild messages.",
                    default: false,
                },
                captureGuildMembers: {
                    type: "boolean",
                    description: "Request Discord's privileged guild-member intent and store member-list identity packets when available.",
                    default: false,
                },
                maxCapturedMessages: {
                    type: "number",
        ...
        passiveIMessageWatch: {
            type: "object",
            description: "Local read-only imsg watch tap that captures iMessage/SMS speakers without turning passive chatter into agent turns.",
            properties: {
                enabled: {
                    type: "boolean",
                    default: false,
        ...
        llmSummarization: {
            type: "object",
            description: "Optional local Ollama fallback summarizer that converts messages into bounded event-note packets.",
            properties: {
                enabled: {
                    type: "boolean",
                    default: false,
                ...
                numCtx: {
                    type: "number",
                    description: "Optional Ollama options.num_ctx override for the fallback summarizer. Leave unset so this background path does not resize the active chat model.",
                    minimum: 512,
                    maximum: MAX_LLM_SUMMARIZATION_NUM_CTX,
                },
                keepAlive: {
                    anyOf: [{ type: "string" }, { type: "number" }, { type: "boolean", const: false }],
                    description: "Optional Ollama keep_alive override for the fallback summarizer. Leave unset, or set false to omit keep_alive.",
                },
                fallbackToPatterns: {
                    type: "boolean",
                    default: true,
        ...
        daemonSummarization: {
            type: "object",
            description: "Optional LibraVDB daemon summarizer used before the LLM fallback for background note extraction.",
            properties: {
                enabled: {
                    type: "boolean",
                    default: false,
        ...
        libravdbProjection: {
            type: "object",
            description: "Optional async projection of passive captured messages into LibraVDB memory.",
            properties: {
                enabled: {
                    type: "boolean",
                    default: false,
                ...
                tenantKey: {
                    type: "string",
                    description: "LibraVDB tenant/user key to project passive captured messages into.",
                },
                pushCapturedMessages: {
                    type: "boolean",
                    description: "Project passive captured messages that were not addressed to the OpenClaw agent.",
                    default: true,
                },
                timeoutMs: {
                    type: "number",
...
let activePassiveDiscordLogTailStop;
let activePassiveDiscordGatewayStop;
let activePassiveIMessageWatchStop;
let activePassiveRuntimeLockRelease;
const activeStores = new Map();
const storeMutationQueues = new Map();
function getUserCardStore(databasePath, legacyJsonPath) {
...
async function readLockTimestamp(lockDir) {
    try {
    ...
    while (true) {
        try {
            ...
            const writeOwner = async () => {
                await writeFile(path.join(params.lockDir, "owner.json"), JSON.stringify({ pid: process.pid, updatedAt: new Date().toISOString() }));
            };
            await writeOwner();
...
class UserCardStore {
    ...
    async load() {
        ...
        const row = db.prepare("select count(*) as count from user_cards").get();
    ...
    async enqueueLibravDBProjection(item) {
        await this.withMutationLock(async () => {
            const db = await this.openDatabase();
            const now = Date.now();
            db.prepare(`
      ...
      `).run(item.id, item.kind, item.sourceId, item.cardKey, item.sessionId, item.sessionKey, item.role, item.content, now, now);
        });
    }
    async claimLibravDBProjectionBatch(limit) {
        ...
        limit ?
      `).all(now, now - LIBRAVDB_PROJECTION_RUNNING_STALE_MS, limit);
    ...
    async completeLibravDBProjection(id) {
        await this.withMutationLock(async () => {
            const db = await this.openDatabase();
            db.prepare(`
        update libravdb_projection_queue
    ...
    async failLibravDBProjection(params) {
        ...
        where id = ?
      `).run(exhausted ? "failed" : "retry", exhausted ? 0 : now + params.retryDelayMs, boundedText(params.error, 500) ?? "projection failed", now, params.id);
        });
    }
    ...
    readFromDatabase() {
        ...
        const cards = {};
        const cardRows = db.prepare("select * from user_cards").all();
        ...
        const eventRows = db.prepare("select * from user_events order by event_when desc").all();
        const causeRows = db.prepare("select event_uuid, caused_by_uuid from user_event_causes").all();
        const causesByEvent = new Map();
        ...
        const messageRows = db.prepare("select * from captured_messages order by at desc").all();
    ...
    async save(store) {
      ...
      `);
            for (const card of Object.values(store.cards)) {
                ...
                insertCard.run(card.key, parsed.provider, parsed.accountId ?? null, parsed.guildId ?? null, parsed.channelId ?? null, parsed.senderId ?? null, normalizeSpeakerKind(card.speakerKind), card.firstSeenAt, card.lastSeenAt, card.messageCount);
                for (const name of card.visibleNames) {
                    insertAlias.run(card.key, name, card.firstSeenAt, card.lastSeenAt, card.messageCount);
                }
                for (const note of card.notes) {
                    insertEvent.run(note.event_uuid, card.key, normalizeEventSignalStrength(note.event_signal_strength, note.event_how), note.event_what, note.event_when, note.event_how);
...
function resolveConfig(value) {
    ...
    return {
        ...
        privateAliases: Array.isArray(record.privateAliases)
            ? record.privateAliases.map((value) => boundedText(value, MAX_ALIAS_CHARS)).filter((value) => Boolean(value))
            : [],
        passiveDiscordLogTail: {
        ...
        llmSummarization: {
            ...
            timeoutMs: clampNumber(llmSummarization.timeoutMs, DEFAULT_LLM_SUMMARIZATION_TIMEOUT_MS, 250, 30_000),
            maxInputChars: clampNumber(llmSummarization.maxInputChars, DEFAULT_LLM_SUMMARIZATION_MAX_INPUT_CHARS, 200, 6_000),
            maxNotesPerMessage: clampNumber(llmSummarization.maxNotesPerMessage, DEFAULT_LLM_SUMMARIZATION_MAX_NOTES, 1, 8),
        ...
        daemonSummarization: {
            ...
            timeoutMs: clampNumber(daemonSummarization.timeoutMs, DEFAULT_DAEMON_SUMMARIZATION_TIMEOUT_MS, 250, 120_000),
            maxOutputTokens: clampNumber(daemonSummarization.maxOutputTokens, DEFAULT_DAEMON_SUMMARIZATION_MAX_OUTPUT_TOKENS, 16, 512),
        },
        libravdbProjection: {
            ...
            timeoutMs: clampNumber(libravdbProjection.timeoutMs, DEFAULT_LIBRAVDB_PROJECTION_TIMEOUT_MS, 250, 120_000),
            retryDelayMs: clampNumber(libravdbProjection.retryDelayMs, DEFAULT_LIBRAVDB_PROJECTION_RETRY_DELAY_MS, 1_000, 3_600_000),
            maxAttempts: clampNumber(libravdbProjection.maxAttempts, DEFAULT_LIBRAVDB_PROJECTION_MAX_ATTEMPTS, 1, 100),
        },
    };
}
function shouldStartPassiveRuntimes(api) {
...
function resolveOptionalOllamaKeepAlive(value) {
    if (value === false) {
        return false;
    }
    if (typeof value === "number" && Number.isFinite(value)) {
...
function normalizeStoreFile(input) {
    const cards = {};
    ...
    for (const [key, card] of Object.entries(inputCards)) {
        ...
        cards[normalizedKey] = {
            ...
            visibleNames: Array.isArray(card.visibleNames)
                ? card.visibleNames.map((name) => boundedText(name, MAX_ALIAS_CHARS)).filter((name) => Boolean(name))
                : [],
            ...
            recentMessages: Array.isArray(card.recentMessages)
                ? card.recentMessages.map(normalizeCapturedMessageRecord).filter((message) => Boolean(message))
                : [],
...
function speakerKindFromAutomation(value) {
    if (value === true) {
        return "agent";
    }
    if (value === false) {
...
function normalizeEventNotes(notes, fallbackTime) {
    return notes.map((note) => {
        ...
        return {
            ...
            event_why: Array.isArray(note.event_why)
                ? note.event_why.map((value) => boundedText(value, 120)).filter((value) => Boolean(value))
                : [],
            event_how: boundedText(note.event_how, 120) ?? "unknown",
        };
    }).filter((note) => Boolean(note));
}
function normalizeEventSignalStrength(value, eventHow) {
...
function defaultSignalStrengthForEventHow(eventHow) {
...
function normalizeCapturedMessageRecord(value) {
...
function firstBoundedString(value, max) {
...
function extractEnvelope(event, ctx) {
    const eventRecord = getRecord(event);
    const ctxRecord = getRecord(ctx);
    const metadata = getRecord(eventRecord.metadata);
    const provider = firstString(ctxRecord.messageProvider, metadata.provider, metadata.channel, eventRecord.channel, "unknown") ?? "unknown";
    const accountId = firstString(eventRecord.accountId, metadata.accountId, ctxRecord.agentId);
    const channelId = firstString(metadata.channelId, eventRecord.conversationId, metadata.originatingTo, ctxRecord.conversationId, eventRecord.channelId, ctxRecord.channelId);
    ...
    const visibleName = firstString(eventRecord.senderName, metadata.senderName, eventRecord.senderUsername, metadata.senderUsername);
    const keyParts = [
        provider,
...
function extractDiscordInboundLogHit(line) {
...
function extractDiscordTokenFromOpenClawConfig(config) {
...
async function readDiscordToken(configPath) {
    try {
...
async function hydrateDiscordAuthor(params) {
    const response = await fetch(`https://discord.com/api/v10/channels/${encodeURIComponent(params.channelId)}/messages/${encodeURIComponent(params.messageId)}`, {
        headers: {
...
async function fetchDiscordGatewayUrl(token) {
...
function parseDiscordGatewayMessage(value) {
    if (typeof value !== "string") {
        return undefined;
    }
    try {
...
function extractDiscordGatewayAuthorEnvelope(event) {
    ...
    const visibleName = sanitizeVisibleName(firstString(member.nick, author.global_name, author.username));
...
function extractDiscordGatewayMentionEnvelopes(event) {
    const record = getRecord(event);
    const channelId = firstString(record.channel_id);
    ...
    for (const value of record.mentions) {
        ...
        seen.add(senderId);
        const visibleName = sanitizeVisibleName(firstString(user.global_name, user.username));
        envelopes.push({
            key: `discord|channel=${channelId}|sender=${senderId}`,
            visibleName,
...
function extractDiscordGatewayGuildMemberEnvelopes(event) {
    const record = getRecord(event);
    ...
    for (const value of record.members) {
        ...
        seen.add(senderId);
        const visibleName = sanitizeVisibleName(firstString(member.nick, user.global_name, user.username));
        envelopes.push({
...
function isDiscordGatewayAgentDirected(event, botUserId) {
    const record = getRecord(event);
...
function extractDiscordGatewayMessageEnvelope(event, cfg) {
...
function parseIMessageWatchLine(value) {
...
function extractIMessageWatchEnvelope(event, cfg) {
    const record = getRecord(event);
    ...
    const chatId = record.is_group === true
        ? firstString(record.chat_guid, typeof record.chat_id === "number" ? String(record.chat_id) : record.chat_id, record.chat_identifier)
        : `imessage:${senderId}`;
    if (!chatId) {
        return undefined;
    }
    const visibleName = sanitizeVisibleName(firstString(record.sender_name, record.sender_display_name, record.display_name));
    const envelope = {
    ...
    const text = sanitizeCapturedMessage(record.text, cfg.passiveIMessageWatch.maxMessageChars ?? 500);
    if (!text) {
        return envelope;
    }
    return {
        ...
        capturedMessage: {
            ...
            messageId: firstString(record.guid, typeof record.id === "number" ? String(record.id) : record.id),
            text,
        },
    };
}
function isIMessageWatchAgentDirected(event) {
    const record = getRecord(event);
...
function sanitizeVisibleName(value) {
    const text = boundedText(value, MAX_ALIAS_CHARS);
...
function sanitizeCapturedMessage(value, maxChars) {
...
function neutralizeControlSyntax(text) {
    return text
...
function isAssistantDirectedMemoryText(text) {
...
function isInstructionLikeMemoryText(text) {
    const normalized = normalizeLookupText(text).replace(/^@\S+\s*/u, "");
    if (!normalized) {
        return false;
    }
...
function isLowInformationMemoryText(text) {
...
function isProviderVisibleNote(note) {
...
function providerVisibleNotes(card) {
    return card.notes
...
function renderProviderVisibleNoteText(value) {
...
function isIdentityEquivalenceMemoryText(text) {
    const normalized = normalizeLookupText(text).replace(/^@\S+\s*/u, "");
    if (!normalized) {
        return false;
    }
...
function diagnosticMemoryTopics(text) {
    const normalized = normalizeLookupText(text);
    const topics = [
...
function extractLearnedNotes(content) {
...
function extractWeakLearnedNotes(content) {
    const text = boundedText(content, 2_000);
    if (!text || containsPrivateData(text)) {
        return [];
    }
...
async function extractLearnedNotesWithLlm(content, cfg, logger) {
    return (await extractLearnedNotesForBackground(content, cfg, logger)).notes;
}
async function extractLearnedNotesForBackground(content, cfg, logger) {
...
function isNonDurableBotQuestion(text) {
    const normalized = normalizeLookupText(text).replace(/^@\S+\s*/u, "");
    if (!normalized) {
        return false;
    }
...
function resolveDaemonSummarizationEndpoint(endpoint) {
...
let daemonSummarizeMessages = async (params) => {
...
let libravDBIngestMessage = async (params) => {
    const isUnix = params.endpoint.startsWith("unix:");
    const socketPath = isUnix ? params.endpoint.slice(5) : undefined;
    const baseUrl = isUnix ? "http://localhost" : params.endpoint.replace(/^tcp:/u, "http://");
    const transport = createGrpcTransport({
...
async function summarizeLearnedNotesWithDaemon(text, cfg, logger) {
    if (cfg.daemonSummarization?.enabled !== true) {
        return undefined;
    }
    const endpoint = resolveDaemonSummarizationEndpoint(cfg.daemonSummarization.endpoint ?? DEFAULT_DAEMON_SUMMARIZATION_ENDPOINT);
    if (!endpoint) {
        return undefined;
    }
    try {
...
function sanitizeDaemonSummary(value) {
...
async function summarizeLearnedNotesWithOllama(text, cfg, logger) {
    ...
    try {
        ...
        return parseLlmEventNotes(firstBoundedString(message.content, 4_000), cfg.llmSummarization.maxNotesPerMessage ?? DEFAULT_LLM_SUMMARIZATION_MAX_NOTES, text);
    }
    catch (error) {
...
function parseLlmEventNotes(content, limit, sourceText) {
    if (!content) {
        return [];
    }
...
function isLlmNoteGroundedInSource(note, sourceText) {
...
function normalizeGroundingToken(token) {
    return token
        .replace(/^['-]+|['-]+$/gu, "")
        .replace(/(?:'s|s)$/u, "");
}
...
function normalizeLlmEventHow(value) {
...
function addCapturedMessageToCard(params) {
    ...
    card.recentMessages = prependUniqueCapturedMessage(card.recentMessages ?? [], params.message, params.maxCapturedMessages);
        ...
        while (pending < MAX_BACKGROUND_SUMMARIES) {
            ...
            void (async () => {
                try {
                    ...
                    await params.store.mutate((store) => addNotesToCard({
                        store,
...
function createLibravDBProjectionScheduler(params) {
    ...
    const endpoint = () => resolveDaemonSummarizationEndpoint(params.cfg.libravdbProjection.endpoint ?? DEFAULT_LIBRAVDB_PROJECTION_ENDPOINT);
    const drain = () => {
        if (wake || pending > 0) {
            return;
        }
    ...
    const drainNow = async () => {
        ...
        try {
            ...
            pending = 0;
            if (processed > 0 && !wake) {
                const retryWake = setTimeout(drain, params.cfg.libravdbProjection.retryDelayMs ?? DEFAULT_LIBRAVDB_PROJECTION_RETRY_DELAY_MS);
                retryWake.unref?.();
            }
        }
    };
    const enqueue = (envelope, agentDirected) => {
        if (!params.cfg.libravdbProjection.enabled ||
...
function createCapturedMessageProjectionItem(envelope, agentDirected) {
...
function prependUniqueCapturedMessage(existing, incoming, limit) {
    const withoutDuplicate = existing.filter((message) => message.messageId
...
async function resolveOpenClawLogPath(configuredPath) {
...
async function startPassiveDiscordLogTail(params) {
    ...
    const logPath = await resolveOpenClawLogPath(params.cfg.passiveDiscordLogTail.logPath ?? DEFAULT_OPENCLAW_LOG_PATH);
    const token = await readDiscordToken(params.cfg.passiveDiscordLogTail.openclawConfigPath ?? DEFAULT_OPENCLAW_CONFIG_PATH);
    if (!token) {
        return undefined;
    }
    let offset = 0;
    ...
    const poll = async () => {
        ...
        for (const line of result.lines) {
            ...
            const visibleName = sanitizeVisibleName(author.displayName ?? author.globalName ?? author.username);
            await params.store.mutate((store) => touchSpeakerIdentity({
                store,
                envelope: {
...
async function startPassiveDiscordGatewayTap(params) {
    ...
    const token = await readDiscordToken(params.cfg.passiveDiscordGateway.openclawConfigPath ?? DEFAULT_OPENCLAW_CONFIG_PATH);
    if (!token) {
        return undefined;
    }
    ...
    params.logger?.info?.(`user-cards passive discord tap starting captureMessages=${params.cfg.passiveDiscordGateway.captureMessages ? "yes" : "no"}`);
    const stop = () => {
        closed = true;
        if (heartbeat) {
            clearInterval(heartbeat);
    ...
    const rememberGatewayEvent = async (event) => {
        ...
        const mentionedEnvelopes = extractDiscordGatewayMentionEnvelopes(event).filter((mentioned) => mentioned.key !== envelope?.key);
        if (!envelope) {
            ...
            if (observedMessages <= 20 || observedMessages % 100 === 0) {
                params.logger?.info?.(`user-cards passive discord tap message ignored count=${observedMessages} reason=no-author-envelope mentions=${mentionedEnvelopes.length}`);
            }
            return;
        }
        observedMessages += 1;
        if (observedMessages <= 20 || observedMessages % 100 === 0) {
            params.logger?.info?.(`user-cards passive discord tap message count=${observedMessages} key=${envelope.key} content=${envelope.capturedMessage ? "stored" : "absent"}`);
        }
        await params.store.mutate((store) => {
    ...
    const rememberGuildMembers = async (event) => {
        ...
        params.logger?.info?.(`user-cards passive discord tap stored guild member identities count=${envelopes.length}`);
    };
    const connect = () => {
        if (closed) {
            return;
        ...
        socket.addEventListener("close", (event) => {
            params.logger?.warn?.(`user-cards passive discord tap closed code=${event.code} reason=${event.reason || "n/a"}`);
            if (heartbeat) {
                clearInterval(heartbeat);
                heartbeat = undefined;
            }
...
function startPassiveIMessageWatch(params) {
    ...
    const scheduleNotes = createNoteScheduler(params);
    const scheduleProjection = createLibravDBProjectionScheduler(params);
    params.logger?.info?.(`user-cards passive imessage watch starting captureMessages=${params.cfg.passiveIMessageWatch.captureMessages ? "yes" : "no"}`);
    const stop = () => {
        closed = true;
        if (reconnect) {
            clearTimeout(reconnect);
    ...
    const rememberWatchEvent = async (event) => {
        const envelope = extractIMessageWatchEnvelope(event, params.cfg);
        if (!envelope) {
            observedMessages += 1;
            if (observedMessages <= 20 || observedMessages % 100 === 0) {
                params.logger?.info?.(`user-cards passive imessage watch message ignored count=${observedMessages} reason=no-speaker-envelope`);
            }
            return;
        }
        observedMessages += 1;
        if (observedMessages <= 20 || observedMessages % 100 === 0) {
            params.logger?.info?.(`user-cards passive imessage watch message count=${observedMessages} key=${envelope.key} content=${envelope.capturedMessage ? "stored" : "absent"}`);
        }
        await params.store.mutate((store) => {
            touchSpeakerIdentity({ store, envelope });
            addCapturedMessageToCard({
...
function prependUnique(values, next, limit) {
    return [next, ...values.filter((value) => value !== next)].slice(0, limit);
}
function prependUniqueEventNote(values, next, limit) {
    const normalized = {
        ...next,
...
function speakerAliasMatchKeys(value) {
...
function isPrivateAlias(value, privateAliases) {
...
function hasSelfIdentityIntent(promptText) {
    const prompt = normalizeLookupText(promptText).replace(/^@+/, "");
...
function promptReferencesCurrentAuthor(promptText, currentCard, cfg) {
    const prompt = normalizeLookupText(promptText).replace(/^@+/, "");
...
function currentUserPromptText(eventRecord) {
...
function stripAddressedSpeakerPrefix(normalizedPrompt, entries) {
...
function renderSpeakerIdentity(card, cfg, options = {}) {
...
function renderCard(card, cfg, options = {}) {
...
function renderChannelRoster(params) {
    ...
    const rosterEntries = params.cards
        .filter((card) => extractChannelKey(card.key) === channelKey &&
        ...
        .filter((entry) => Boolean(entry?.rendered));
    const uniqueEntries = rosterEntries.filter((entry, index, entries) => entries.findIndex((candidate) => candidate.rendered === entry.rendered) === index).slice(0, params.cfg.maxRosterNames);
    ...
    if (hasSelfIdentityIntent(params.promptText) ||
...
function renderMatchingEvents(params) {
    ...
    const matches = candidateCards
        .flatMap((card) => providerVisibleNotes(card).map((note) => ({
        card,
        note,
...
function selectRecallEventMatches(matches, limit) {
    ...
    [...matches]
        ...
        .forEach(add);
    [...matches]
        .sort((a, b) => b.note.event_signal_strength - a.note.event_signal_strength ||
        b.score - a.score ||
...
function findReferencedSpeakerIdentities(cards, prompt) {
...
function getSpeakerIdentityKey(cardKey) {
...
function renderEventMemoryLine(prefix, card, note, cfg, options = {}) {
...
function renderNoteObservationText(text, sourceSpeaker) {
...
function renderEventWhere(cardKey, currentChannel, includeSourceDetails = false) {
    const parsed = parseCardKey(cardKey);
...
function hasRecallIntent(promptText) {
...
function scoreEventMatch(params) {
    ...
    const speakerIdentity = normalizeLookupText([extractSenderKey(params.card.key), ...params.card.visibleNames].filter(Boolean).join(" "));
...
export const internals = {
    ...
    setDaemonSummarizeMessagesForTest(fn) {
    ...
    setLibravDBIngestMessageForTest(fn) {
...
const plugin = {
    ...
    register(api) {
        ...
        async function capture(event, ctx) {
        ...
        api.on("before_prompt_build", async (event, ctx) => {
            ...
            const prependContext = joinContextSections(renderChannelRoster({
                cards,
